...
class TestMethods {
    ...
    void multiply1(Double firstNumber, Double secondNumber) {
        final Methods test = new Methods(firstNumber, secondNumber, "multiply");
    ...
    void divide1(Double firstNumber, Double secondNumber) {
        final Methods test = new Methods(firstNumber, secondNumber, "divide");
        final Double actualResult = test.calculate();
        final Double expectedResult = (test.getFirstDoubleNumber() / test.getSecondDoubleNumber());
    ...
    @CsvSource(value = {"12,8", "22, 4", "21, 6"})
    @DisplayName("Subtract test using CsvSource(Positive)")
    @ParameterizedTest(name = "Subtract between {0} and {1}")
    void subtract1(Integer firstNumber, Integer secondNumber) {
        final Methods test = new Methods(firstNumber, secondNumber, "subtract");
        final Double actualResult = test.calculate();
        final Double expectedResult = (double) test.getFirstNumber() - test.getSecondNumber();
        MatcherAssert.assertThat("actual result matches expected result", actualResult, is(expectedResult));
    }
    ...
    void methodsUsing1(Double firstNumber, Double secondNumber, String method, Matcher<Double> matcher) {
        final Methods test = new Methods(firstNumber, secondNumber, method);
        final Double actualResult = test.calculate();
